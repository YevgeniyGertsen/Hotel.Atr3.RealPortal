...
namespace Hotel.Atr3.Admin.Controllers
{
    public class HomeController : Controller
    {
        ...
        public HomeController(ILogger<HomeController> logger)
        {
            ...
            //ApiUrl = _configuration.GetSection("ApiUrl").GetValue<string>("RealApi");
        }

        public async Task<IActionResult> Position()
        {
            List<Position> positions = new List<Position>();
            string path = Path.Combine(ApiUrl, "Position");
        ...
        //    {
        //        string result = await request.Content.ReadAsStringAsync();
        //        teams = Newtonsoft.Json.JsonConvert.DeserializeObject<List<Team>>(result);
        //    }
